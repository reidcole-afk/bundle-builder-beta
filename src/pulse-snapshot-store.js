...
function sanitizeCoin(input = {}) {
  return {
    ...
    pulseScore: finiteOrNull(input.pulseScore),
    confidenceScore: finiteOrNull(input.confidenceScore),
    confidenceLabel: safeText(input.confidenceLabel, 40),
    fragilityScore: finiteOrNull(input.fragilityScore),
    signalLane: safeText(input.signalLane, 40),
    graduationBoost: finiteOrNull(input.graduationBoost),
    graduationLabel: safeText(input.graduationLabel, 40),
    graduationFlags: Array.isArray(input.graduationFlags) ? input.graduationFlags.slice(0, 6).map((flag) => safeText(flag, 60)).filter(Boolean) : [],
    graduatesFromWatchlist: Boolean(input.graduatesFromWatchlist),
    rankMomentum: finiteOrNull(input.rankMomentum),
    wakeUpScore: finiteOrNull(input.wakeUpScore),
    wakeUpLabel: safeText(input.wakeUpLabel, 40),
    wakeUpFlags: Array.isArray(input.wakeUpFlags) ? input.wakeUpFlags.slice(0, 6).map((flag) => safeText(flag, 60)).filter(Boolean) : [],
    confirmedWakeUpScore: finiteOrNull(input.confirmedWakeUpScore),
    confirmedWakeUpLabel: safeText(input.confirmedWakeUpLabel, 40),
    confirmedWakeUpFlags: Array.isArray(input.confirmedWakeUpFlags) ? input.confirmedWakeUpFlags.slice(0, 6).map((flag) => safeText(flag, 60)).filter(Boolean) : [],
    regimeFit: finiteOrNull(input.regimeFit),
    regime: safeText(input.regime, 40),
